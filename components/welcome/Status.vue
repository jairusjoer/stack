...
const refresh = async () => {
  try {
    ...
    console.log('Directus', 'refreshed status');
  } catch (error) {
    health.value = null;
    console.error('Directus', 'failed to refresh status:', error);
  }
};
</script>

<template>
  <div class="inline-flex overflow-hidden rounded border font-mono text-xs leading-6 font-medium">
    <span class="px-2">Directus Status</span>
    <button
      title="Click to refresh"
      :class="{
        'group flex cursor-pointer items-center gap-2 rounded-[3px] px-2 uppercase': true,
